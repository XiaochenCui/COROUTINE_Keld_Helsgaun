...
void Coroutine::Enter()
{
    ...
    if (Ready)
    {
        ...
        // environment of tmp_jmpb saved here
        if (!setjmp(tmp_jmpb))
    ...
    { // activate control block
        ...
        // jump to the routine of the coroutine
        longjmp(MyTask->jmpb, 1);
    }
}
...
void Coroutine::Eat()
{
    ...
    if (MyTask->next != &t)
    {
        // insert t between MyTask and MyTask->next
...
{
    // find the first stack block
    std::cout << "Stack layout: " << std::endl;
    std::cout << main_task.prev << " " << &main_task << " " << main_task.next << std::endl;
}
